...
`infrastructure/` ha ricevuto lo stesso trattamento — vedi nota datata nelle Conseguenze qui sotto: la decisione iniziale era diversa e si è rivelata sbagliata nell'uso reale.
...
> - **Aggiornamento 29/08/2026 — la decisione originale su `infrastructure/` era sbagliata:** questo ADR, nella sua prima stesura, escludeva `infrastructure/` (la copia di lavoro di `ansible/`+`opentofu/`) dal versionamento per intero, motivandolo con "lì dentro non c'è contenuto originale da preservare, solo artefatti di esecuzione". Falso in pratica: dentro ci sono i `.tf` e i ruoli Ansible reali, non rigenerabili. Risultato concreto: aggiungendo un nuovo host (`Paca-120`, per PACA) una modifica a `opentofu/nodes/dell-emc/variables.tf` tracciata a repo-root non arrivava da nessuna parte finché qualcuno non copiava a mano il file dentro `/opt/infrastructure/opentofu/nodes/dell-emc/` su `pve-management` — `tofu apply` continuava silenziosamente a pianificare sullo stato vecchio, nessun errore, nessun avviso. **Fix**: `ansible/` e `opentofu/` spostati con `git mv` sotto `infrastructure/ansible/` e `infrastructure/opentofu/`, tracciati direttamente come `docker-compose/` (punto 1 sopra) — lo sparse-checkout di `pve-management` aveva già il cono `infrastructure` incluso, quindi `git reset --hard origin/main` ora li sincronizza da solo, stessa via già usata per `docker-compose/`. Solo gli artefatti di esecuzione (`terraform.tfstate*`, `.terraform/`, `collections/`, `inventory.yml`, il virtualenv `.venv-1password/`) restano esclusi dal repository, non più le due cartelle intere.
